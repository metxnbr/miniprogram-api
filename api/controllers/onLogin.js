...
const getSession = async ({ url, options }) => {
  const json = await axios.get(url, options);
  const { openid, session_key, unionid, expires_in } =
    (json && json.data) || {};
...
const jwtToken = async (data, options) => {
  const token = await jwt.sign(data, options);
  return token;
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (currentSessionId) {
      ...
      const access_token = await jwtToken(
        { openid, session_key },
